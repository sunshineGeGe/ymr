$(document).ready(function(){
	...
	$("#big-pic").html("<img src='"+url+"' onload='loadpic("+next_picid+")'><div id=\"BlowupLens\" ></div>");
	$('#big-pic img').LoadImage(true, 890, 650,$("#load_pic").attr('rel'));
	$("#big-pic img").css({"background": "url("+url+") no-repeat" });
...
function showpic(type, replay) {
	...
	if(type=='next' || type=='pre') {
		...
		$("#big-pic").html("<img src='"+url+"' onload='loadpic("+next_picid+")'><div id=\"BlowupLens\" ></div>");
		$('#big-pic img').LoadImage(true, 890, 650,$("#load_pic").attr('rel'));
		// $("#BlowupLens").css({"background": "url("+url+")" });
		$("#big-pic img").css({"background": "url("+url+") no-repeat" });
...
function loadpic(id) {
	...
	$("#load_pic").html("<img src='"+url+"'><div id=\"BlowupLens\" ></div>");
	$("#BlowupLens").css({"background": "url("+url+") no-repeat" });
}